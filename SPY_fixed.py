...
def calculate_wealth(df, initial_investment=10000, allow_short_selling=False):
    ...
    for i in range(len(df)):
        ...
        if signal == 1 and (position == 0 or (allow_short_selling and position == -1)):
...
def evaluate_combination(params, df, initial_investment, allow_short_selling=False):
    ...
    final_wealth, trades, long_trades, short_trades = calculate_wealth(df_copy, initial_investment, allow_short_selling)
...
def run_analysis(file_path, start_date, end_date, short_min, short_max, long_min, long_max, 
               allow_short_selling=False, progress_var=None, root=None, run_button=None, progress_bar=None):
    ...
    try:
        ...
        for long_ma in long_windows:
            for short_ma in short_windows:
                ...
                final_wealth, trades, long_trades, short_trades = calculate_wealth(df_tmp, initial_investment, allow_short_selling)
        ...
        if not df_custom.empty:
            ...
            custom_wealth, custom_trades, custom_long_trades, custom_short_trades = calculate_wealth(df_custom, initial_investment, allow_short_selling)
            if custom_trades:
...
def launch_gui():
    ...
    def run():
        ...
        threading.Thread(target=run_analysis, args=(file_path, start_date, end_date, short_min, short_max, long_min, long_max, 
                                                   allow_short_selling, progress_var, root, run_button, progress_bar)).start()
