...
def show_price_chart(file_path, start_date, end_date):
    ...
    # Create a Text widget for the trade list
    trades_detail = tk.Text(list_frame, height=20, width=60, font=("Courier", 10),
                           bg='#f5f5f5', relief=tk.SUNKEN, bd=1)
    trades_detail.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)

    # Add vertical scrollbar
    detail_scrollbar = tk.Scrollbar(list_frame, orient=tk.VERTICAL, command=trades_detail.yview)
    detail_scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
    trades_detail.config(yscrollcommand=detail_scrollbar.set)
    # Define controls_container and controls_scroll before usage
    ...
    # Create a horizontal frame for MA controls and slope controls
    ma_controls_row = tk.Frame(controls_frame, bg='#f0f0f0')
    ma_controls_row.pack(fill=tk.X, pady=(10, 0))

    # Short MA control
    short_ma_frame = tk.Frame(ma_controls_row, bg='#f0f0f0')
    short_ma_frame.pack(side=tk.LEFT, padx=(0, 10), fill=tk.Y)
    ...
    # Long MA control
    long_ma_frame = tk.Frame(ma_controls_row, bg='#f0f0f0')
    long_ma_frame.pack(side=tk.LEFT, padx=(0, 10), fill=tk.Y)
    ...
    # Slope controls (Include Slope Check and Lookback)
    slope_controls_frame = tk.Frame(ma_controls_row, bg='#f0f0f0')
    slope_controls_frame.pack(side=tk.LEFT, padx=(10, 0), fill=tk.Y)
    include_slope_check_cb = tk.Checkbutton(slope_controls_frame, text="Include Slope Check",
        ...
        bg='#f0f0f0')
    include_slope_check_cb.pack(side=tk.LEFT, padx=(0, 5))
    tk.Label(slope_controls_frame, text="Lookback:", bg='#f0f0f0').pack(side=tk.LEFT)
    lookback_spinbox = tk.Spinbox(slope_controls_frame, from_=1, to=10, width=3,
    ...
    def update_chart():
        ...
        buy_offset = price_range * 0.08  # 8% of price range above price
        sell_offset = price_range * 0.08  # 8% of price range below price
        ...
        # Set y-limits to ensure markers are visible
        padding = (max_price - min_price) * 0.12
        ax.set_ylim(min_price - padding*3.0, max_price + padding*3.0)

        # Set legend with correct labels
        ax.legend(['Price', f'Short MA ({short_ma})', f'Long MA ({long_ma})'], loc='upper left', fontsize=9)
        ...
        ax.legend(['Price', f'Short MA ({short_ma})', f'Long MA ({long_ma})'], loc='upper left', fontsize=9)
            ...
            # Calculate strategy performance and trades using calculate_wealth
            ...
            # Restore original Recent Trades display (no Gain% column)
            trades_text.config(state=tk.NORMAL)
            trades_text.delete(1.0, tk.END)
            if all_trades:
                for t in all_trades[-5:]:
                    action, date, price = t[:3]
    ...
    def show_detailed_trades():
            ...
            list_frame.pack(fill=tk.BOTH, expand=True)
                ...
                # Add Gain% column to Complete Trade History window
                ...
                position = 0  # 0: no position, 1: long position, -1: short position
                last_action = None
                # Track if simulated closing trade has been added
                ...
                for i, trade in enumerate(all_trades):
                    ...
                    # Format aligned row with Comments, previous MA columns, and both Slope columns, with vertical bars
                    row_text = (
                ...
                # After all trades, if a position is open and simulated closing trade not yet added, add it once
                if position != 0 and not simulated_trade_added:
                    simulated_trade_added = True
                    # Get the final row from df_copy
                    ...
                    prev_short_ma_str = f"{prev_short_ma:.2f}" if not np.isnan(prev_short_ma) else "N/A"
                    prev_long_ma_str = f"{prev_long_ma:.2f}" if not np.isnan(prev_long_ma) else "N/A"
                    # Calculate Gain% for simulated closing trade
                    gain_str = ""
                    if entry_price is not None:
                        if position == 1:
                    ...
                    if position == 1:
                        ...
                        tag = "cover"
                        position_info = "(Simulated closing trade)"
                    # Use same alignment as other rows, including Gain%, with vertical bars
                    row_text = (
    ...
    # Create the matplotlib Figure before embedding
    fig = plt.Figure(figsize=(8, 5), dpi=100)
    ax = fig.add_subplot(111)

    # Embed the plot in the Tkinter window
    # Define chart_frame before using it for the Matplotlib canvas
