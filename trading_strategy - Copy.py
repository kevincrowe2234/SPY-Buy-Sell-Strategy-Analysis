...
class TradingStrategy:
    ...
    def read_csv(self, file_path):
        """
        Read and validate CSV file with date and price data.
        ...
        try:
            ...
            df = pd.read_csv(file_path, header=None, names=['Date', 'Price'],
                           sep=',', engine='python')
            logging.info(f"Read data with shape: {df.shape}")
            ...
            try:
                ...
                logging.info("Date conversion successful with day/month/year format")
            except ValueError:
                try:
                    df['Date'] = pd.to_datetime(df['Date'], format='%m/%d/%Y')
                    logging.info("Date conversion successful with month/day/year format")
            ...
            if df['Date'].isna().any():
                ...
                logging.warning(f"{invalid_count} rows have invalid dates")
...
class TradingGUI:
    ...
    def setup_ui(self):
        ...
        file_frame.pack(pady=2, padx=10, fill=tk.X)
        tk.Entry(file_frame, textvariable=self.file_path_var, width=60).pack(side=tk.LEFT)
        tk.Button(file_frame, text="Browse", command=self.browse_file, width=15).pack(side=tk.RIGHT)
    ...
    def _run_backtest_worker(self):
        ...
        try:
            ...
            # Save results
            self._save_results(results_df, params, best_result)
    ...
    def _save_results(self, results_df, params, best_result):
