...
all_trades = []  # Assuming this is defined somewhere in the file

def show_detailed_trades():
    global all_trades
    if not all_trades or len(all_trades) == 0:
        messagebox.showinfo("No Trades", "No trades to display.")
        return
    win = tk.Toplevel()
    win.title("Detailed Trade List")
    win.geometry("1600x700")
    frame = tk.Frame(win)
    frame.pack(fill=tk.BOTH, expand=True)
    text = scrolledtext.ScrolledText(frame, font=("Courier New", 12), wrap=tk.NONE)
    text.pack(fill=tk.BOTH, expand=True)
    # Heading
    text.insert(tk.END, "\n")
    text.insert(tk.END, "Complete Trade History".center(150) + "\n\n")
    # Table header
    header = (
        f"{'Action':<8}|{'Date':<12}|{'Price':<8}|{'Gain%':<8}|{'Prev Short MA':<14}|{'Short MA':<10}|"
        f"{'Prev Long MA':<14}|{'Long MA':<10}|{'Short MA Slope':<15}|{'Long MA Slope':<15}|{'Comments':<30}|\n"
    )
    ...
    for trade in all_trades:
        ...
        color = 'green' if action.startswith('Buy') else ('red' if action.startswith('Sell') else 'black')
        comment = trade.get('Comments','')
        comment_color = 'red' if 'Simulated closing trade' in comment else 'green' if 'Opening' in comment else 'black'
        line = (
            f"{action:<8}|{trade.get('Date',''):<12}|{trade.get('Price',''):<8.2f}|{trade.get('Gain%',''):<8}|"
            f"{trade.get('Prev Short MA',''):<14}|{trade.get('Short MA',''):<10}|{trade.get('Prev Long MA',''):<14}|{trade.get('Long MA',''):<10}|"
            f"{trade.get('Short MA Slope',''):<15}|{trade.get('Long MA Slope',''):<15}|{comment:<30}|\n"
        )
        ...
        # Color action
        text.tag_add(action, start_idx, f"{start_idx}+8c")
        text.tag_config('Buy', foreground='green')
        text.tag_config('Sell', foreground='red')
        # Color comment
        comment_start = f"{start_idx}+{len(line)-30}c"
        text.tag_add('comment', comment_start, end_idx)
...
- Buy → Sell (for long trades)
- Short Sell → Cover Short (for short trades)
The last trade may not have a matching closing trade if the position is still open at the end of the simulation. If so, a simulated closing trade marked with an asterisk (*) is shown.

*Note: Trades with an * are not actual trades, they indicate a trade is still open at the end of the simulation.
Note: positive slope means the MA is trending upward, while a negative slope means it is trending downward.
"""
                ...
                if "Gain" in value_label:
                    ...
                    try:
                        ...
                        if os.path.exists(combined_file):
                            ...
                            # Find the row with the exact MA values
                            ma_row = combined_data[(combined_data['short_ma'] == short_ma) & 
                                                   (combined_data['long_ma'] == long_ma)]
                            

                            if not ma_row.empty:
                                # Get the additional values
                                total_trades = ma_row['trade_count'].values[0]
                                avg_gain = ma_row['annualized_gain'].values[0]
                                
                                # Update the tooltip text
                                tooltip_text += (f"\nTotal Trades: {total_trades}\n"
                                                 f"Average Annualized Gain: {avg_gain:.2f}%")
...
def show_price_chart(file_path, start_date, end_date):
    ...
    global short_ma_var, long_ma_var, filtered_df
    # Initialize strategy performance variables if not already
    if 'trades_count_var' not in globals() or trades_count_var is None:
        trades_count_var = tk.IntVar(value=0)
    if 'long_trades_var' not in globals() or long_trades_var is None:
        long_trades_var = tk.IntVar(value=0)
    if 'short_trades_var' not in globals() or short_trades_var is None:
        short_trades_var = tk.IntVar(value=0)
    if 'annualized_gain_var' not in globals() or annualized_gain_var is None:
        annualized_gain_var = tk.DoubleVar(value=0.0)
    import pandas as pd
    try:
        ...
        debug_info = f"First 5 rows:\n{filtered_df.head().to_string()}\n\nDate dtype: {filtered_df['Date'].dtype}\nPrice dtype: {filtered_df['Price'].dtype}"
        from tkinter import messagebox
        messagebox.showinfo("Chart Data Preview", debug_info)
        print(f"Loaded {len(filtered_df)} rows for chart. Columns: {list(filtered_df.columns)}")
        if filtered_df.empty:
            debug_info = f"No chart data found.\nRows loaded: {len(df)}\nColumns: {list(df.columns)}\nDate range: {start_date} to {end_date}\nFile: {file_path}"
            messagebox.showerror("Error", debug_info)
            return
    except Exception as e:
        messagebox.showerror("Error", f"Failed to load chart data: {str(e)}")
        return

    # Create chart window and frames for layout
    chart_window = tk.Toplevel()
    chart_window.title("Price Chart")
    chart_window.geometry("1200x600")
    main_frame = tk.Frame(chart_window)
    main_frame.pack(fill=tk.BOTH, expand=True)
    chart_frame = tk.Frame(main_frame)
    chart_frame.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
    controls_frame = tk.Frame(main_frame, padx=20, pady=10)
    ...
    def update_chart():
        ...
        try:
            ...
            messagebox.showerror("Error", f"Moving average periods must be positive integers.\n{str(e)}")
            return
        if filtered_df.empty:
            messagebox.showerror("Error", "Price data is not loaded.")
            return
        ...
        # Calculate trades and update global all_trades and performance variables
        global all_trades, trades_count_var, long_trades_var, short_trades_var, annualized_gain_var
        allow_short = allow_short_selling_var.get()
        short_only = short_sell_only_var.get() if allow_short else False
        final_wealth, annualized_gain, trade_list = calculate_wealth(df_copy, initial_investment=10000, allow_short_selling=allow_short, short_sell_only=short_only)
        # Build detailed trade list
        all_trades = []
        long_trades = 0
        short_trades = 0
        prev_short_ma = prev_long_ma = prev_price = None
        for idx, trade in enumerate(trade_list):
            ttype, tdate, tprice = trade
            df_idx = df_copy.index[df_copy['Date'] == tdate]
            if len(df_idx) == 0:
                continue
            i = df_idx[0]
            short_ma = df_copy['Short_MA'].iloc[i]
            long_ma = df_copy['Long_MA'].iloc[i]
            short_ma_slope = df_copy['Short_MA_Slope'].iloc[i] if 'Short_MA_Slope' in df_copy else 'NA'
            long_ma_slope = df_copy['Long_MA_Slope'].iloc[i] if 'Long_MA_Slope' in df_copy else 'NA'
            gain = 0.0
            comment = ''
            if ttype == 'Buy':
                ...
                if prev_price is not None:
                    gain = ((tprice - prev_price) / prev_price) * 100
            elif ttype == 'Short Sell':
                comment = '(Opening short position)'
            elif ttype == 'Cover Short':
                comment = '(Closing short position)'
                if prev_price is not None:
                    gain = ((prev_price - tprice) / prev_price) * 100
            trade_dict = {
                'Action': ttype,
                'Date': str(tdate)[:10],
                'Price': float(tprice),
                'Gain%': round(gain, 2),
                'Prev Short MA': round(prev_short_ma, 2) if prev_short_ma is not None else '',
                'Short MA': round(short_ma, 2) if not pd.isna(short_ma) else '',
                'Prev Long MA': round(prev_long_ma, 2) if prev_long_ma is not None else '',
                'Long MA': round(long_ma, 2) if not pd.isna(long_ma) else '',
                'Short MA Slope': round(short_ma_slope, 2) if isinstance(short_ma_slope, (int, float)) else short_ma_slope,
                'Long MA Slope': round(long_ma_slope, 2) if isinstance(long_ma_slope, (int, float)) else long_ma_slope,
                'Comments': comment
            }
            ...
            if ttype in ('Buy', 'Sell'):
                long_trades += 1
            elif ttype in ('Short Sell', 'Cover Short'):
                short_trades += 1
        trades_count_var.set(len(trade_list))
        long_trades_var.set(long_trades)
        short_trades_var.set(short_trades)
        annualized_gain_var.set(annualized_gain)
        ...
        ax.set_xlabel('Date')
        ax.set_ylabel('Price')
        ax.set_title(f'Price Chart with Buy/Sell Signals ({start_date}) to ({end_date})')
        padding = (max_price - min_price) * 0.12
        ax.set_ylim(min_price - padding*3.0, max_price + padding*3.0)
        ax.grid(True, alpha=0.3)
        ax.legend(loc='upper left', fontsize='small', frameon=True, fancybox=True)
        legend_text = "Signal Legend:\nB = Buy (Short MA crosses above Long MA)\nS = Sell (Short MA crosses below Long MA)"
    ...
    # Chart Settings controls in the right column
    tk.Label(controls_frame, text="Chart Settings", font=("Arial", 12, "bold")).pack(pady=10)
    ma_frame = tk.Frame(controls_frame)
    ma_frame.pack(pady=5, fill=tk.X)
    tk.Label(ma_frame, text="Short MA:").grid(row=0, column=0, sticky=tk.W)
    tk.Entry(ma_frame, textvariable=short_ma_var, width=8).grid(row=0, column=1)
    tk.Label(ma_frame, text="Long MA:").grid(row=1, column=0, sticky=tk.W)
    tk.Entry(ma_frame, textvariable=long_ma_var, width=8).grid(row=1, column=1)
    tk.Checkbutton(ma_frame, text="Include Slope Check", variable=include_slope_check_var, command=update_chart).grid(row=2, column=0, sticky=tk.W, pady=2)
    tk.Label(ma_frame, text="Lookback:").grid(row=2, column=1, sticky=tk.W)
    tk.Entry(ma_frame, textvariable=slope_lookback_var, width=8).grid(row=2, column=2)
    tk.Button(ma_frame, text="Update Chart", command=update_chart, width=15).grid(row=3, column=0, columnspan=3, pady=10)

    # Short Selling Options
    options_frame = tk.LabelFrame(controls_frame, text="Short Selling Options", padx=10, pady=5)
    options_frame.pack(fill=tk.X, pady=10)
    allow_short_cb = tk.Checkbutton(options_frame, text="Allow Short Selling", variable=allow_short_selling_var, command=update_chart)
    allow_short_cb.pack(anchor=tk.W)
    short_sell_only_cb = tk.Checkbutton(options_frame, text="Short Sell Only", variable=short_sell_only_var, state=tk.NORMAL if allow_short_selling_var.get() else tk.DISABLED, command=update_chart)
    short_sell_only_cb.pack(anchor=tk.W)

    # Strategy Performance
    perf_frame = tk.LabelFrame(controls_frame, text="Strategy Performance", padx=10, pady=5)
    perf_frame.pack(fill=tk.X, pady=10)
    perf_labels = {
        'Annualized %Gain': tk.Label(perf_frame, text="Annualized %Gain: "),
        'Number of Trades': tk.Label(perf_frame, text="Number of Trades: "),
        'Long Trades': tk.Label(perf_frame, text="Long Trades: "),
        'Short Trades': tk.Label(perf_frame, text="Short Trades: ")
    }
    for i, key in enumerate(perf_labels):
        perf_labels[key].pack(anchor=tk.W)

    def update_performance():
        # Dummy values for now; replace with actual calculations if available
        perf_labels['Annualized %Gain'].config(text=f"Annualized %Gain: {getattr(annualized_gain_var, 'get', lambda: 'N/A')():.2f}%" if hasattr(annualized_gain_var, 'get') else "Annualized %Gain: N/A")
        perf_labels['Number of Trades'].config(text=f"Number of Trades: {getattr(trades_count_var, 'get', lambda: 'N/A')()}" if hasattr(trades_count_var, 'get') else "Number of Trades: N/A")
        perf_labels['Long Trades'].config(text=f"Long Trades: {getattr(long_trades_var, 'get', lambda: 'N/A')()}" if hasattr(long_trades_var, 'get') else "Long Trades: N/A")
        perf_labels['Short Trades'].config(text=f"Short Trades: {getattr(short_trades_var, 'get', lambda: 'N/A')()}" if hasattr(short_trades_var, 'get') else "Short Trades: N/A")

    # View All Trades button
    tk.Button(controls_frame, text="View All Trades", command=show_detailed_trades).pack(pady=10)
...
def launch_gui():
    ...
    global short_ma_var, long_ma_var
    ...
    short_ma_var = tk.IntVar(value=10)  # Default value for short MA
    long_ma_var = tk.IntVar(value=150)  # Default value for long MA
